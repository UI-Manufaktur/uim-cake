...
import uim.baklava.database.Expression\CommonTableExpression;
import uim.baklava.database.Expression\IdentifierExpression;
import uim.baklava.database.Expression\OrderByExpression;
import uim.baklava.database.Expression\OrderClauseExpression;
import uim.baklava.database.Expression\QueryExpression;
import uim.baklava.database.Expression\ValuesExpression;
import uim.baklava.database.Expression\WindowExpression;
import uim.baklava.database.Statement\CallbackStatement;
use Closure;
...
class Query : IExpression, IteratorAggregate
{
    ...
    function leftJoin(myTable, $conditions = [], myTypes = []) {
        this.join(this._makeJoin(myTable, $conditions, static::JOIN_TYPE_LEFT), myTypes);
    ...
    function rightJoin(myTable, $conditions = [], myTypes = []) {
        this.join(this._makeJoin(myTable, $conditions, static::JOIN_TYPE_RIGHT), myTypes);
    ...
    function innerJoin(myTable, $conditions = [], myTypes = []) {
        this.join(this._makeJoin(myTable, $conditions, static::JOIN_TYPE_INNER), myTypes);
    ...
    function enableBufferedResults(bool myEnable = true) {
        this._dirty();
        this._useBufferedResults = myEnable;
            ...
            if (is_array($part)) {
                foreach ($part as $i => $piece) {
                    if (is_array($piece)) {
                        ...
                        /** @psalm-suppress PossiblyUndefinedMethod */
                        this._parts[myName][$i] = clone $piece;
                    }
                }
            }
            if ($part instanceof IExpression) {
                this._parts[myName] = clone $part;
            }
        }
    }
