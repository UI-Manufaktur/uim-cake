...
class Text
{
     ...
     * @return string
     */
    static function insert(string $str, array myData, array myOptions = []): string
    {
        $defaults = [
     ...
     */
    static function cleanInsert(string $str, array myOptions): string
    {
     ...
     */
    static function wrap(string $text, myOptions = []): string
    {
     ...
     * @return string Formatted text.
     */
    static function wrapBlock(string $text, myOptions = []): string
    {
        if (is_numeric(myOptions)) {
            myOptions = ['width' => myOptions];
        }
     ...
     * @return string Formatted text.
     */
    protected static auto _wordWrap(string $text, int $width = 72, string $break = "\n", bool $cut = false): string
    {
        $parts = [];
        if ($cut) {
     ...
     */
    static function highlight(string $text, $phrase, array myOptions = []): string
    {
        if (empty($phrase)) {
            return $text;
        }
     ...
     */
    static function tail(string $text, int $length = 100, array myOptions = []): string
    {
        $default = [
            'ellipsis' => '...', 'exact' => true,
        ];
     ...
     * @return string
     */
    protected static auto _substr(string $text, int $start, ?int $length, array myOptions): string
    {
        if (empty(myOptions['trimWidth'])) {
            $substr = 'mb_substr';
        } else {
