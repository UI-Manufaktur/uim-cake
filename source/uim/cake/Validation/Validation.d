...
class Validation
{
     ...
     */
    static function creditCard($check, myType = 'fast', bool $deep = false, ?string $regex = null): bool
    {
     ...
     * @return bool Success
     */
    static function ip($check, string myType = 'both'): bool
    {
        if (!is_string($check)) {
            return false;
        }
    ...
    static function iban($check): bool
    {
        ...
        $numStr = str_replace($search, $replace, $account . myCountry . '00');
     ...
     * @return string
     */
    protected static auto _getDateString(array myValue): string
    {
        $formatted = '';
        if (
